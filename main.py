...
def make_batches(x_src, y_src, batch_size):
    ...
    batches_num = int(len(x) / int(batch_size))
...
def encoder_data(batches_num, x_batches, y_batches, word_vec, word_vec_len, word_vec_size):
    ...
    for i in range(batches_num):
        ...
        for sentence in x:
            ...
            while len(tmp) < int(word_vec_len):
                tmp.append(np.zeros(int(word_vec_size), dtype=np.float32))
            tmp = np.array(tmp)
...
if __name__ == '__main__':
    ...
    word_vec_len = config.getint('hyperparameters', 'word_vec_len')
    ...
    # 数据预处理
    train_x_batches, train_y_batches, train_batches_num = make_batches("./data/0_train_X.npy", "./data/0_train_Y.npy", batch_size)
    train_x, train_y, train_len = encoder_data(train_batches_num, train_x_batches, train_y_batches, word_vec,
                                               word_vec_len,
                                               word_vec_size)
    test_x_batches, test_y_batches, test_batches_num = make_batches("./data/0_test_X.npy", "./data/0_test_Y.npy", batch_size)
